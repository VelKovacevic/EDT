...
public class RemoveEnseignant extends JFrame implements ActionListener{
    ...
    public RemoveEnseignant(Seance seance, LocalDate debut, int id, String nom, int type){
        this.seance = seance;
        this.debut = debut;
        this.id = id;
        this.nom = nom;
        this.type = type;
        this.setTitle("Enlever un enseignant");
        ...
        if(seance != null){
            if(seance.getEnseignants()!= null){
                ensInfo = new Object[seance.getEnseignants().length][2];
        
                for(int i=0; i<seance.getEnseignants().length; ++i){
                    ensInfo[i][0] = seance.getEnseignants()[i].getNom();
                    ensInfo[i][1] = seance.getEnseignants()[i].getPrenom();
                }

                groupeTable = new JTable(ensInfo, title){
                    @Override
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == back){
            ...
            if(selection.length > 0){
                String nomA = groupeTable.getValueAt(selection[0], 0).toString();
                String prenomA = groupeTable.getValueAt(selection[0], 1).toString();
                ...
                try{
                    String sql = "SELECT\n" +
                                    ...
                                    "	utilisateur\n" +
                                    "WHERE\n" +
                                    "	utilisateur.NOM = \"" + nomA + "\" AND utilisateur.PRENOM = \""+ prenomA +"\"";
                    ...
                    if(idGrp != 0){
                        sql = "DELETE FROM seance_enseignants WHERE `ID_SEANCE`= " + seance.getId() + " AND `ID_ENSEIGNANT` = " + idGrp;
                        stmt.executeUpdate(sql);
                        int a=0;
                        Enseignant[] tempsa = new Enseignant[seance.getEnseignants().length-1];
                        for(int i=0; i<seance.getEnseignants().length; ++i){
                            
                            if(i == selection[0])
                                --a;
                            else{
                                tempsa[a] = seance.getEnseignants()[i];
                            }
                            ++a;
                        }
                        seance.setEnseignants(tempsa);
                        new RemoveEnseignant(seance, debut, id, nom, type);
                        this.setVisible(false);
                        this.dispose();
                    }
