 ...
 * @author kovac
 */
public class ListEdtAdmin extends JFrame implements ActionListener{
    private final int SIZE_X = 930, SIZE_Y = 700;
    ...
    private JButton AddSeance = new JButton("Add Seance");
    private JButton annule = new JButton("Annuler");
    private JButton RmvGrp = new JButton("Remove Grp.");
    private JButton RmvEns = new JButton("Remove Ens.");
    private JButton editNom = new JButton("Edit Nom");
    private JButton DeplSeance = new JButton("Deplacer Seance");
    
    private JTable coursTable;
    
    public ListEdtAdmin(Seance[] seancesAnnule, Seance[] seances, LocalDate debut, int id, String nom, int type){
        ...
        AddEns.addActionListener(this);
        AddGrp.addActionListener(this);
        AddSalle.addActionListener(this);
        AddSeance.addActionListener(this);
        annule.addActionListener(this);
        RmvGrp.addActionListener(this);
        ...
        if(seances != null){
            ...
            coursTable = new JTable(cours, titles){
                @Override
        ...
        LocalDate temporalDate = debut.minusDays(7*9);
        debutDates = new LocalDate[19];
        for(int i=1; i<=19; i++){
        ...
        this.getContentPane().add(das, BorderLayout.SOUTH);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource() == deconnexion){
            ...
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == AddEns){ // ADD ENSEIGNANT
            int[] selection = coursTable.getSelectedRows();
            new AddEnseignant(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == AddGrp){
            int[] selection = coursTable.getSelectedRows();
            new AddGroupe(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == editNom){
            int[] selection = coursTable.getSelectedRows();
            new EditNom(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == AddSalle){
            int[] selection = coursTable.getSelectedRows();
            new AddSalle(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == RmvGrp){
            int[] selection = coursTable.getSelectedRows();
            new RemoveGroupe(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == RmvEns){
            int[] selection = coursTable.getSelectedRows();
            new RemoveEnseignant(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
        }else if(e.getSource() == DeplSeance){
            int[] selection = coursTable.getSelectedRows();
            new MoveSeance(seances[selection[0]], debut, id, nom, type);
            this.setVisible(false);
            this.dispose();
            ...
            int[] selection = coursTable.getSelectedRows();
            Seance seance = seances[selection[0]];
            ...
            try{
                String sql = "UPDATE `seance` SET `ETAT` = '2' WHERE `seance`.`ID` = " + seance.getId();
                stmt.executeUpdate(sql);
                switch (type) {
                    case 1:
                        {
                            seances = Funtions.seancesUtilisateur(id, 1, debut, debut.plusDays(6));
                            new ListEdtAdmin(seancesAnnule, seances, debut, id, nom, type);
                            this.setVisible(false);
                            this.dispose();
                            break;
                    ...
                    case 2:
                        {
                            seances = Funtions.seancesEnseignant(id, 1, debut, debut.plusDays(6));
                            new ListEdtAdmin(seancesAnnule, seances, debut, id, nom, type);
                            this.setVisible(false);
                            this.dispose();
                            break;
                    ...
                    case 3:
                        {
                            seances = Funtions.seancesSalle(id, 1, debut, debut.plusDays(6));
                            new ListEdtAdmin(seancesAnnule, seances, debut, id, nom, type);
                            this.setVisible(false);
                            this.dispose();
                            break;
